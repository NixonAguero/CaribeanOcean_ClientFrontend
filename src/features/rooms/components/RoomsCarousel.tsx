...
export default function RoomsCarousel({ rooms }: Props) {
  ...
  return (
    <div className={styles.carousel}>
      <button onClick={prev} className={`${styles.arrow} ${styles.leftArrow}`}>
        ‹
      </button>
      ...
      <div className={styles.track}>
        {rooms.map((room, index) => {
          ...
          return (
            <div key={room.id} className={`${styles.item} ${styles[position]}`}>
      ...
      <button onClick={next} className={`${styles.arrow} ${styles.rightArrow}`}>
        ›
      </button>
    </div>
  );
